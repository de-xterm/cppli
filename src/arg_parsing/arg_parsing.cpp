            ...
            if(is_valid_subcommand(subcommand_name, arg_string)) {
                ...
                if(arg_string.substr(0,2) == "--") { // long flag (these are ez)
                    ...
                    for(unsigned char_i = 1; char_i < arg_string.size(); ++char_i) {
                        ...
                        else if(!subcommand_option_argument_is_optional(subcommand_name, arg_string.substr(char_i,1))) { // FIXME this means you can't do -abcblue where -c's argument is optional and -a and -b are flags
